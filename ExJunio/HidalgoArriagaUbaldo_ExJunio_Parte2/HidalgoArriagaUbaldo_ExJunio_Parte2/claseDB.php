...
class DB {
    ...
    public static function buscaPatron($patron){
        $connection=self::conexion();
        global $resultBusqueda;
        //$expresion = "%".$patron."%";
        try {
            $consulta = $connection->prepare("SELECT * FROM producto where nombre LIKE CONCAT('%', :patron, '%') or descripcion LIKE CONCAT('%', :patron, '%') ");
            ...
            if ($busqueda) {
                ...
                do{
                    $busquedas[] = array('codigo' => $busqueda['cod'],'nombre' =>$busqueda['nombre'],'descripcion' =>$busqueda['descripcion'],'PVP' =>$busqueda['PVP'],'familia' =>$busqueda['familia'],'stock' =>$busqueda['stock']);                
                } while ($busqueda = $consulta->fetch());
            }  
    ...
    public static function buscaPersonalizada($patron){
        $connection=self::conexion();
        global $resultBusqueda;
        //$expresion = "%".$patron."%";
        try {
    ...
    if (isset($_POST['search']) && !empty($_POST['search'])){
        ...
        $buscar = DB::buscaPatron($_POST['search']);
        echo  json_encode($buscar);

    }elseif (isset($_POST['accionProducto']) && $_POST['accionProducto']=="insertar"){
